...
public class Parser {
    ...
    public Query handleUpdateStatement(ZUpdate s, TransactionId tid)
            throws TransactionAbortedException, DbException, IOException,
            simpledb.ParsingException, Zql.ParseException {
        int id;
        try {
        ...
        if (s.getWhere() != null)
            processExpression(tid, (ZExpression) s.getWhere(), lp);
        lp.addProjectField("null.*", null); // is this necessary?

        @SuppressWarnings("unchecked")
        Hashtable<String, ZConstant> updates = s.getSet();
        HashMap<String, Field> tupUpdates = new HashMap<String, Field>();
        for (String col : updates.keySet()) {
            ZConstant zVal = updates.get(col);
            Field fVal;
            if (zVal.getType() == 2) {
                fVal = new IntField(Integer.parseInt(zVal.getValue()));
            }
            else if (zVal.getType() == 3) {
                fVal = new StringField(zVal.getValue(), Type.STRING_LEN);
            }
            else {
                throw new simpledb.ParsingException("Unknown update value type: " + zVal.getValue());
            }
            tupUpdates.put(col, fVal);
        }
        DbIterator op = new Update(tid, lp.physicalPlan(tid,
                TableStats.getStatsMap(), explain), tupUpdates);
        sdbq.setPhysicalPlan(op);
            ...
            if (s instanceof ZTransactStmt)
                ...
                try {
                    if (s instanceof ZInsert)
                        ...
                        query = handleUpdateStatement((ZUpdate) s,
                                curtrans.getId());
                    else {
                        System.out
                                .println("Can't parse "
    ...
    protected void start(String[] argv)
        ...
        if (!interactive) {
            try {
                ...
                FileInputStream fis = new FileInputStream(new File(queryFile));
                int ch;
                while((ch = fis.read()) != -1){
            ...
            while (!quit && (line = reader.readLine("SimpleDB> ")) != null) {
                ...
                while (line.indexOf(';') >= 0) {
                    ...
                    if (cmd.toLowerCase().contains("read only;") && cmd.toLowerCase().contains("transaction")) {
                        System.out.println("Read Only not supported in current implementation - converting to `read write`");
                        cmd = "set transaction read write;"; // this is lip service
                    }
                    if (cmd.equalsIgnoreCase("### crash;")) {
                        Utility.crashAndRestart();
                        curtrans = null;
    ...
    protected void processInput(String cmd) throws IOException {
